...
class OneDriveReader(BaseReader):
    ...
    def _authenticate_with_msal(self) -> Any:
        ...
        if self._is_interactive_auth:
            ...
            app = msal.PublicClientApplication(self.client_id, authority=self._authority)
            ...
            app = msal.ConfidentialClientApplication(self.client_id, authority=self._authority, client_credential=self.client_secret)
    ...
    def _construct_endpoint(
        ...
        if not self._is_interactive_auth and not userprincipalname:
            raise Exception("userprincipalname cannot be empty for App authentication. Provide the userprincipalname (email mostly) of user whose onderive needs to be accessed")
    ...
    def _get_items_in_drive_with_maxretries(
        ...
        endpoint = self._construct_endpoint(item_ref, isRelativePath, isFile, userprincipalname)
        ...
        while retries < max_retries:
            ...
            # very frequently for large amount of file
                ...
                logger.warning(f"Retrying {retries+1} in {retries+1} secs. Status code: {response.status_code}")
                retries += 1
                time.sleep(retries)  # Exponential back-off
            else:
                raise Exception(f"API request to download {item_ref} failed with status code: {response.status_code}, message: {response.content}")
    ...
    def _extract_metadata_for_file(
        ...
        props = {
            ...
            'last_modified_by_app': modified_by.get("application", {}).get("displayName"),
    ...
    def _check_and_download_file_from_onedrive(self, item: Dict[str, Any], local_dir: str, mime_types: Optional[List[str]] = None):
        ...
        accepted_mimetypes = [mimetype.lower() for mimetype in mime_types] if mime_types else ["*"]

        # Check if the item's MIME type is among the accepted MIME types
        is_accepted_mimetype = "*" in accepted_mimetypes or item['file']['mimeType'].lower() in accepted_mimetypes

        if is_accepted_mimetype:
            # It's a file with an accepted MIME type; download and extract metadata
            file_path = self._download_file_by_url(item, local_dir)  # Assuming this method is implemented
            metadata[file_path] = self._extract_metadata_for_file(item)  # Assuming this method is implemented
        else:
            # Log a debug message for files that are ignored due to an invalid MIME type
            logger.debug(f"Ignoring file '{item['name']}' as its MIME type does not match the accepted types.")
    ...
    def _download_files_from_onedrive(
        ...
        data = self._get_items_in_drive_with_maxretries(access_token, item_id, userprincipalname = userprincipalname, isRelativePath=isRelativePath)
    ...
    def _load_documents_with_metadata(
        ...
        loader = SimpleDirectoryReader(directory, file_metadata=get_metadata, recursive=recursive)
