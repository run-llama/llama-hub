...
class FuzzyCitationQueryEngine(CustomQueryEngine):
    ...
    def get_relevant_sentences(self, response: RESPONSE_TYPE, threshold: int = DEFAULT_THRESHOLD) -> Dict[str, Dict[str, Any]]:
        ...
        for source_node in response.source_nodes:
            ...
            node_sentences = tokenizer(node.get_content(metadata_mode=MetadataMode.NONE))
            ...
            for (response_sent_idx, node_sent_idx) in sorted(top_sentences.keys()):
                if prev_idx == -1:
                    top_chunks[response_sent_idx].append(top_sentences[(response_sent_idx, node_sent_idx)])
                elif node_sent_idx - prev_idx == 1:
                    top_chunks[response_sent_idx][-1] += top_sentences[(response_sent_idx, node_sent_idx)]
                else:
                    top_chunks[response_sent_idx].append(top_sentences[(response_sent_idx, node_sent_idx)])
            ...
            for response_sent_idx, chunks in top_chunks.items():
                for chunk in chunks:
                    start_char_idx = node.get_content(metadata_mode=MetadataMode.NONE).find(chunk)
                    end_char_idx = start_char_idx + len(chunk)

                    response_sent = response_sentences[response_sent_idx]
                    results[(response_sent, chunk)] = {"start_char_idx": start_char_idx, "end_char_idx": end_char_idx, "node": node}
    ...
    def custom_query(self, query_str: str) -> RESPONSE_TYPE:
        ...
        fuzzy_citations = self.get_relevant_sentences(response, threshold=self.threshold)
...
class FuzzyCitationEnginePack(BaseLlamaPack):
    def __init__(self, query_engine: BaseQueryEngine, threshold: int = DEFAULT_THRESHOLD) -> None:
        """Init params."""
        try:
            from thefuzz import fuzz
        except ImportError:
        ...
        self.query_engine = FuzzyCitationQueryEngine(query_engine=query_engine, threshold=threshold)

    def get_modules(self) -> Dict[str, Any]:
        """Get modules."""
        return {"query_engine": self.query_engine, "query_engine_cls": FuzzyCitationQueryEngine}
