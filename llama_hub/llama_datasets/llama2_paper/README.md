...
You can download `Llama2PaperDataset` directly using `llamaindex-cli`, which comes installed with the `llama-index` python package:
...
rag_evaluate_pack.run(dataset=rag_dataset, query_engine=query_engine)
```
