...
To use it, a "classic" personal access token with the `read:org` and `read:project` scopes is required. See [here](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token) for instructions.
...
download_loader("GitHubRepositoryIssuesReader")
...
if docs is None:
    github_client = GitHubIssuesClient(os.getenv("GITHUB_TOKEN"))
    loader = GitHubRepositoryIssuesReader(
        github_client,
        owner =                  "jerryjliu",
        repo =                   "llama_index",
