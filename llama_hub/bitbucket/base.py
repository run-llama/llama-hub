...
class BitbucketReader(BaseReader):
    ...
    def get_slugs(self) -> List:
      ...
      repos_url = f"{self.base_url}/rest/api/latest/projects/{self.project_key}/repos/"
      headers = {
    ...
    def load_all_file_paths(self, slug, branch, directory_path='', paths=[]):
      ...
      for value in children['values']:
        if (value['type'] == 'FILE'):
          paths.append({
              'slug': slug,
              'path': f'{directory_path}/{value["path"]["name"]}'
              })
        elif (value['type'] == 'DIRECTORY'):
          self.load_all_file_paths(slug=slug, branch=branch, directory_path=f'{directory_path}/{value["path"]["name"]}', paths=paths)
    ...
    def load_text(self, paths) -> List :
      ...
      for path in paths:
        ...
        lines_list = load_text_by_paths(self.base_url, self.project_key, path['slug'], path['path'], self.branch)
        concatenated_string = ""
    ...
    def load_data(self) -> List[Document]:
        ...
        paths = []
        for slug in slugs:
          self.load_all_file_paths(slug=slug, branch=self.branch, directory_path='', paths=paths)
