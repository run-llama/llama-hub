...
from tenacity import retry, stop_all, stop_after_attempt, stop_after_delay, wait_random_exponential
...
class RayyanReader(BaseReader):
    ...
    def __init__(self, credentials_path: str, rayyan_url: str = 'https://rayyan.ai') -> None:
    ...
    def load_data(self, review_id: str, filters: dict = {}) -> List[Document]:
        ...
        logging.info(f"Working on review: '{my_review['title']}' with {my_review['total_articles']} total articles.")
        ...
        articles = []
        logging.info(f"Fetching articles from Rayyan...")
        ...
        for article in articles:
            ...
            if article["abstracts"] is not None:
              abstracts_arr = [abstract["content"] for abstract in article["abstracts"]]
